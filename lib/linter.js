...
function rawIgnoreRegex(html, options) {
  var ignore = options["raw-ignore-regex"];
  if (!ignore) {
    return html;
  }
...
class Linter {
  ...
  lint(html) {
    ...
    Object
      ...
      .forEach(name => {
        ...
        if (rule.lint) {
          let ruleIssues = this.executeRule(rule, { dom, lines });
  ...
  lintByTag(rule, dom) {
    function executeLint(node) {
        ...
        let chilrenIssues = node.children.map(executeLint.bind(this));
  ...
  callRuleLint (rule, raw) {
    return rule.need? rule.lint(raw, this.config.legacy_config, this.config.activatedRules) : rule.lint(raw, rule.config);
  }
