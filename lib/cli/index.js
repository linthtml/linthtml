...
async function lint(input, config_path) {
  let files_linters = [];
  const searchSpinner = ora({ text: "Searching for files", stream: process.stdout }).start();
  try {
  ...
  const lintSpinner = ora({ text: "Analysing files", stream: process.stdout });
  try {
    lintSpinner.start();
    ...
    lintSpinner.fail();
    console.error();
    console.error(chalk`An error occured while analysing {underline ${error.fileName}}`);
    console.error();
    printErrors(error);
...
function printReports(reports) {
  ...
  if (reports.length > 0) {
    ...
    if (errorsCount > 0) {
      console.error(chalk`{red ✖ ${problemsCount} ${problemsCount > 1 ? "problems" : "problem"} (${errorsCount} ${errorsCount > 1 ? "errors" : "error"}, ${warningCount} ${warningCount > 1 ? "warnings" : "warning"})}`);
      displayBetaVersionMessage();
      return exitProcess(true);
    }
