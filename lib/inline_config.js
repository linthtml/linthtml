  ...
  config.rules.forEach(
    function (rule) {
      ...
      if (rule.type === "rule") {
        ...
        var preset = isprev ? this.previousPreset : presets.presets[rule.value];
...
function parsePair(name, value, pos, isOption) {
  if (!name || !value || !name.length || !value.length) {
    throw new Error(`Cannot parse inline configuration.`, {
      pos
    });
  }
  ...
  var squote = "'",
    dquote = '"'; // Single and double quote, for sanity
  ...
  if (value === "$previous") {
    ...
    try {
      ...
      if (!nameRegex.test(value)) {
        throw new Error(`Inline configuration for rule "${name}" cannot be parsed.`, {
          pos
        });
      }
      parsed = value;
...
class InlineConfig {
   ...
   */
  constructor(config) {
    // this.setOption = config.setRuleConfig.bind(config);
  ...
  feedComment(element) {
    var line = element.data,
      match = line.match(/[\s]*linthtml-configure[\s]+(.*)/);
