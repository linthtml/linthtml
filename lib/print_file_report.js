...
module.exports = function print_file_report(report) {
  console.error(chalk`File: {underline ${report.fileName}}`);
  if (report.config_path) {
    console.error(chalk`{blue Config file: {white.underline ${report.config_path}}}`);
  }
  if (report.preset) {
    console.error(chalk`{blue Using preset: {white ${report.preset}}}`);
  }
