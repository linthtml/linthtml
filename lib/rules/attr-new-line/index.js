...
module.exports.lint = function(node, opts, { report }) {
  ...
  node.attributes.forEach((attribute) => {
    ...
    // }
    const line = attributes_lines.get(attribute.loc.start[0]) || [];
    attributes_lines.set(attribute.loc.start[0], line.concat([attribute]));
    if (attribute.loc.end[0] !== attribute.loc.start[0]) {
      const line = attributes_lines.get(attribute.loc.end[0]) || [];
      attributes_lines.set(attribute.loc.end[0], line.concat([attribute]));
    }
  });
  ...
  const tag_line = node.openLineCol[0];
