...
function check_indent_width_close({ openLineCol, close }) {
  if (close === undefined) {
    return true;
  }
  if (openLineCol.line === close.loc.start.line) {
    return true;
  }
  return openLineCol.column === close.loc.start.column;
}
...
function sort_children(a, b) {
  if (a.lineCol.line < b.lineCol.line) {
    return -1;
  }
  if (a.lineCol.line < b.lineCol.line) {
    return 1;
...
function children_on_same_line(node) {
  return node.children.every(child => child.lineCol.line === node.lineCol.line);
}

function one_node_per_line(nodes) {
  return nodes.slice(1).reduce((prev, current) => {
    if (prev[prev.length - 1].lineCol.line !== current.lineCol.line) {
      prev.push(current);
    }
    return prev;
  }, nodes.slice(0, 1));
      ...
      report({
        ...
        meta: {
          data: {
            ...
            current_indentation: node.indent.length,
            isClose: false
          }
        }
      });
