...
function check_indents(node, indent_style, expected_indent_width, indent_width) {
  ...
  if (node.children && node.children.length > 0) {
    ...
    children = children.slice(1).reduce((prev, current) => {
      if (prev[prev.length - 1].lineCol[0] !== current.lineCol[0]) {
        prev.push(current);
      }
      return prev;
    }, children.slice(0,1));
    children.forEach(child => issues = issues.concat(check_indents(child, indent_style, (expected_indent_width + indent_width), indent_width)));
  }
...
// TODO: Clean dom and keep only one node per line (the first)
