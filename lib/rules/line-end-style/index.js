...
module.exports.lint = function(node, opts, { report }) {
  ...
  let offset = 0;
  while ((match = R.exec(node.data)) !== null) {
    const EOL = match[0];
    lines.push({
      offset: offset++,
      text: node.data.slice(previous_index, match.index + EOL.length)
    });
    previous_index = match.index + EOL.length;
  }

  if (previous_index !== node.data.length) {
    lines.push({
      offset,
      text: node.data.slice(previous_index)
    });
  }
