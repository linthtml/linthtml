...
module.exports.lint = function(line, opts) {
  ...
  if (deltaEnabled && !cont) {
    ...
    if (
      !(
        expectedMin <= currentLineIndentCount &&
        currentLineIndentCount <= expectedMax
      )
    ) {
      output.push(
        new Issue("E056", [line.row], {
  ...
  if (width && !cont) {
    let count = 0;
    let index = 0;
    for (index = 0; index < indent.length; index++) {
      var c = indent[index];
      if (c === " ") {
        count++;
      } else {
    ...
    if (count % width !== 0) {
      output.push(new Issue("E036", [line.row, index - count + 1], { width: width }));
    }
  }
  ...
  if (format) {
    ...
    // if (error) {
    //   output.push(
    //     new Issue("E024", [line.row, error.index + 1], {
