...
describe("title-max-len", function() {
  ...
  it("Should not report any error when the title does exceed max length", function(done) {
    const linter = createLinter();
    const html = `<head><title>Title!</title></head>`;
    
    linter.lint(html, none, { "title-max-len": 60 }).then((issues) => {
      expect(issues).to.have.lengthOf(0);
      done();
    }); 
  });

  it("Should not report any error when the title length equal the max length", function(done) {
    const linter = createLinter();
    const html = `<head><title>Title</title></head>`;
    
    linter.lint(html, none, { "title-max-len": 5 }).then((issues) => {
      expect(issues).to.have.lengthOf(0);
      done();
    }); 
  });

  it("Should report an error when the title does exceed the max length", function(done) {
    const linter = createLinter();
    const html = `<head><title>Title!</title></head>`;
