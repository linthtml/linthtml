...
describe("line-max-len", function() {
  ...
  it("Should not report any error when the line does not exceed the max length", function(done) {
    const linter = createLinter();
    const html = `1234`;
    
    linter.lint(html, none, { "line-max-len": 5 }).then((issues) => {
      expect(issues).to.have.lengthOf(0);
      done();
    }); 
  });

  it("Should not report any error when the line length equal the max length", function(done) {
    const linter = createLinter();
    const html = `12345`;
  ...
  it("Should support multilines", function(done) {
    const linter = createLinter();
    const html = `12345\n12345\n1234`;
  ...
  it("Should report an error when the line does exceed the max length", function(done) {
    const linter = createLinter();
    const html = `123456`;
