...
function augment_config(cosmiconfig_result) {
  ...
  // not needed here
  // if (config.plugins) {
  //   const normalized_plugins = Array.isArray(config.plugins) // throw an error if not string or array
  //     ? config.plugins
  //     : [config.plugins];
  ...
  result = add_plugins_rules(result); // Should be after augment_config only // not here
  return result;
}
/**
...
function add_plugins_rules(cosmiconfig_result) {
  if (cosmiconfig_result.config.plugins) {
    ...
    const plugins_rules = normalized_plugins.reduce((rules, plugin_name) => {
      let plugin_import;
      try {
        plugin_import = require(plugin_name);
      } catch (error) {
        throw new CustomError("CORE-05", { module_name: plugin_name });
      }

      // Handle either ES6 or CommonJS modules
      plugin_import = plugin_import.default || plugin_import;

      // A plugin can export either a single rule definition
      // or an array of them
      const normalized_plugin_import = Array.isArray(plugin_import)
        ? plugin_import
        : [plugin_import];

      normalized_plugin_import.forEach((rule_definition) => {
        ...
        rules[rule_definition.name] = rule_definition;
      });
