module.exports.CORE_ERRORS = {
  ...
  "06": (chalk, meta) => chalk`{red Error:} Plugins needs expose rules that have a property "name". The plugin "${meta.plugin_name}" is not doing this, so it will not work. Please file an issue with the plugin.`,
  ...
  "08": (chalk, meta) => chalk`{red Error:} Plugin rules needs to define a "lint" function. The plugin rule "${meta.rule_name}" from plugin "${meta.plugin_name}" does not do this, so will not work. Please file an issue with the plugin.`
};
