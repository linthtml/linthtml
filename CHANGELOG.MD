...
- Rule `inpu-req-label` no longer report an error for input with type "hidden".
- Rule `inpu-req-label` no longer report an error for input with type "button" when the attribut value has a non empty value.
