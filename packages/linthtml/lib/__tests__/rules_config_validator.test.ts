...
describe("Rules config validators", () => {
  describe("String|Regexp validator", () => {
    ...
    [1, () => {}, [], {}, null, undefined, true].forEach((_) => {
      it("validation fn throw an error if a string or a regexp is not provided in input", () => {
        const fn = create_string_or_regexp_validator("foo");
        expect(() => fn(_)).toThrow(
    ...
    ["", /a/].forEach((_) => {
      it("validation fn does not throw an error if a string or a regexp is provided in input", () => {
        const fn = create_string_or_regexp_validator("foo");
    ...
    it("can create a validator that does not allow empty string value", () => {
      ...
      expect(() => fn("")).toThrow('Configuration for rule "foo" is invalid: You provide an empty string value.');
    });
  });
  ...
  describe("Numbers validator", () => {
    ...
    ["foo", /a/, () => {}, [], {}, null, undefined, true].forEach((_) => {
      it("validation fn throw an error if a number is not provided in input", () => {
        const fn = create_number_validator("foo");
        expect(() => fn(_)).toThrow(`Configuration for rule "foo" is invalid: Expected number got ${typeof _}.`);
      });
    });

    [1, -1].forEach((_) => {
      it("validation fn does not throw an error if a number is provided in input", () => {
        const fn = create_number_validator("foo");
        expect(() => fn(_)).not.toThrow();
      });
    });
    ...
    it("can create a validator that does not negative numbers", () => {
      ...
      expect(() => fn(-1)).toThrow('Configuration for rule "foo" is invalid: Only positive indent value are allowed.');
    });
  });

  describe("String list validators", () => {
    it("create validator function throw an error if nothing is provide in input", () => {
      // @ts-expect-error Testing rule validation
      expect(() => create_list_value_validator("foo")).toThrow("You must provide a array of string");
    });

    it("create validator function throw an error if not provided with an array of string", () => {
      // @ts-expect-error Testing rule validation
  ...
  describe("Boolean validator", () => {
    ...
    [1, () => {}, [], {}, null, undefined, "", /a/].forEach((_) => {
      it("validation fn throw an error if a boolean is not provided in input", () => {
